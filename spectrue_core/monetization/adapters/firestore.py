...
class FirestoreBillingStore(BillingStore):
    ...
    # =========================================================================
    # V3 Ledger-First Charging Methods
    # =========================================================================

    def apply_charge_v3_transactional(
        self,
        uid: str,
        run_id: str,
        amount_sc: Decimal,
        ...
        """
        Transactional charge with Ledger V3 first.
        - Idempotent: returns existing result if key exists.
        - Atomic: update balance + create ledger.
        - Split: available_sc first, then balance_sc.
        """
        ...
        @firestore.transactional
        def _charge_tx(transaction):  # type: ignore[no-untyped-def]
            # 1. Check Ledger V3 for Idempotency
            # Using new collection: billing_ledger_v3
            ledger_ref = self._db.collection("billing_ledger_v3").document(idempotency_key)
            ledger_snap = ledger_ref.get(transaction=transaction)

            if ledger_snap.exists:
                entry = LedgerEntry.from_dict(ledger_snap.to_dict())

                if entry.status == LedgerStatus.SETTLED:
                    split_data = entry.split or {}
                    new_wallet = entry.new_wallet or {}
                    return ChargeResult(
                        ok=True,
                        split=ChargeSplit(
                            take_available=MoneySCClass.from_str(split_data.get("take_available_sc", "0")),
                            take_balance=MoneySCClass.from_str(split_data.get("take_balance_sc", "0")),
                        ),
                        new_balance_sc=MoneySCClass.from_str(new_wallet.get("balance_sc", "0")),
                        new_available_sc=MoneySCClass.from_str(new_wallet.get("available_sc", "0")),
                    )
                elif entry.status == LedgerStatus.FAILED:
                    return ChargeResult(
                        ok=False,
                        split=ChargeSplit(take_available=MoneySCClass.zero(), take_balance=MoneySCClass.zero()),
                        new_balance_sc=MoneySCClass.zero(), # Unknown in failure state, or need fetch
                        new_available_sc=MoneySCClass.zero(),
                        reason=entry.reason or "previous_failure",
                    )
                # If PENDING, we might want to retry or fail. Assuming retry here behaves as new attempt.

            # 2. Fetch User Data
            user_ref = self._users.document(uid)
            user_snap = user_ref.get(transaction=transaction)
            if not user_snap.exists:
                # User not found -> Failed Ledger
                entry = LedgerEntry(
                    idempotency_key=idempotency_key,
                    uid=uid,
                    run_id=run_id,
                    entry_type=LedgerTypeV3.CHARGE_V3,
                    amount_sc=MoneySCClass(amount_sc),
                    status=LedgerStatus.FAILED,
                    created_at=datetime.now(timezone.utc),
                    reason="user_not_found"
                )

                transaction.set(ledger_ref, entry.to_dict())
                return ChargeResult(
                    ok=False,
                    split=ChargeSplit(take_available=MoneySCClass.zero(), take_balance=MoneySCClass.zero()),
                    new_balance_sc=MoneySCClass.zero(),
                    new_available_sc=MoneySCClass.zero(),
                    reason="user_not_found",
                )

            data = user_snap.to_dict() or {}
            current_bal = _to_decimal(data.get(self._config.balance_field, "0"))
            current_avail = _to_decimal(data.get(self._config.available_field, "0"))

            # 3. Calculate Split
            # Available first
            take_avail = min(current_avail, amount_sc)
            rem = amount_sc - take_avail
            # Balance second
            take_bal = min(current_bal, rem)
            rem = rem - take_bal
            ...
            if rem > 0:
                ...
                entry = LedgerEntry(
                    ...
                    created_at=datetime.now(timezone.utc),
                    reason="insufficient_funds",
                    meta={"required": str(amount_sc), "has_avail": str(current_avail), "has_bal": str(current_bal)}
                )
                ...
                return ChargeResult(
                    ok=False,
                    split=ChargeSplit(take_available=MoneySCClass.zero(), take_balance=MoneySCClass.zero()),
                    new_balance_sc=MoneySCClass(current_bal),
                    new_available_sc=MoneySCClass(current_avail),
                    reason="insufficient_funds",
                )

            # 5. Execute Charge
            new_avail = quantize_sc(current_avail - take_avail)
            new_bal = quantize_sc(current_bal - take_bal)
            ...
            entry = LedgerEntry(
                ...
                created_at=datetime.now(timezone.utc),
                split={
                    "take_available_sc": str(take_avail),
                    "take_balance_sc": str(take_bal),
                },
                new_wallet={
                    "available_sc": str(new_avail),
                    "balance_sc": str(new_bal),
                }
            )
            ...
            transaction.set(
                user_ref,
                {
                    self._config.available_field: str(new_avail),
                    self._config.balance_field: str(new_bal),
                },
                merge=True,
            )
            
            # --- Bonus Ledger (Spend Tracking) ---
            # If available_sc was used (bonus credits), record in bonus_ledger
            if take_avail > 0:
                 bonus_entry = BonusLedgerEntry(
                    uid=uid,
                    event_type="spend",
                    amount_sc=take_avail * -1,
                    date=datetime.now(timezone.utc),
                    related_run_id=run_id,
                    balance_after=MoneySCClass(new_avail),
                )
            ...
            return ChargeResult(
                ok=True,
                split=ChargeSplit(
                    take_available=MoneySCClass(take_avail),
                    take_balance=MoneySCClass(take_bal),
                ),
                new_balance_sc=MoneySCClass(new_bal),
                new_available_sc=MoneySCClass(new_avail),
            )

        return _charge_tx(transaction)
