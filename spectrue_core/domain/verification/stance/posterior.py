...
from spectrue_core.domain.verification.stance.model import (
    StanceFeatures,
    StancePosterior,
)
...
def compute_stance_posterior(features: StanceFeatures) -> StancePosterior:
    ...
    feature_values = {
        ...
        "llm_said_refute": 1.0
        if llm_stance_lower in ("refute", "ref", "contradict")
        else 0.0,
        "quote_present": 1.0 if features.quote_present else 0.0,
        "has_chunk": 1.0 if features.has_evidence_chunk else 0.0,
        "relevance": (
            features.llm_relevance if features.llm_relevance is not None else 0.5
...
def compute_effective_evidence_count(posteriors: list[StancePosterior]) -> dict:
    ...
    return {
        ...
        "mean_entropy": round(
            sum(p.entropy for p in posteriors) / max(len(posteriors), 1), 3
        ),
    }
